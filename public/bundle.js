...
GeometryEditor.prototype.editInternal = function editInternal (geometry, operation) {
  ...
  if (geometry instanceof Point$2) {
...
OffsetCurveSetBuilder.prototype.add = function add (g) {
  ...
  else if (g instanceof Point$2) { this.addPoint(g); }
...
PointExtracter.prototype.filter = function filter (geom) {
  if (geom instanceof Point$2) { this._pts.add(geom); }
};
...
ConnectedElementLocationFilter.prototype.filter = function filter (geom) {
  if (geom instanceof Point$2 || geom instanceof LineString || geom instanceof Polygon) { this._locations.add(new GeometryLocation(geom, 0, geom.getCoordinate())); }
};
...
GeometryTransformer.prototype.transform = function transform (inputGeom) {
  ...
  if (inputGeom instanceof Point$2) { return this.transformPoint(inputGeom, null) }
...
function turf_buffer(geojson, radius, opts) {
    return buffer$2(geojson, radius, opts);
}
...
function vectors(blob, cb) {
    ...
    blobToBuffer(blob, function(err, buf) {
        ...
        var tile = new VectorTile$1(new pbf(buf));
